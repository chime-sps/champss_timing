...
parser.add_argument("--show-details", action="store_true", default=False, help="Show details of a file. (specified by the pulsar id and archive id)")
...
if args.auto_insert_raw_data:
    ...
    # Run action
    cli_masterdb_hdl.insert_data(placeholder_if_corrupted=args.placeholder_if_corrupted, psr=args.psr)
    ...
    if args.psr is None or args.arid is None:
        logger.error("Please specify both pulsar id and archive id to show details. ")
    else:
        cli_masterdb_hdl.show_details(psr_id=args.psr, ar_id=args.arid)
    ...
    if args.psr is not None and args.arid is not None:
        cli_masterdb_hdl.show_details(psr_id=args.psr, ar_id=args.arid)
    else:
