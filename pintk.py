...
parser.add_argument("--dealiased-par", action="store_true", default=False, help="Use dealiased parfile instead of fitted parfile")
...
with database.database(f"{psrdir}/champss_timing.sqlite3.db", readonly=True) as db_hdl:
    ...
    print(f"Generating par file at: {parfile_path} (using {'initial' if args.initial_par else 'dealiased' if args.dealiased_par else 'fitted'} parfile)")
    pintk_utils.write_parfile(parfile_path, use_initial=args.initial_par, use_dealiased=args.dealiased_par)
