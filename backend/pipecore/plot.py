...
class plot:
    ...
    def initialize(self):
        ...
        self.toas = self.db_hdl.get_all_toas()

        if self.db_loaded:
            self.db_hdl.close()

        toas_file_idxed = {}
        for this_toa in self.toas:
            self.toas_mjdint_idxed[int(this_toa["toa"])] = this_toa
    ...
    def get_plot_data(self):
        ...
        plot_data["stacked_amps"] = np.sum(plot_data["amps_normalized"], axis=0)
    ...
    def diagnostic(self, savefig=None, show_ephms=True):
        ...
        if show_ephms:
            ...
            fig, axs = plt.subplots(4, 4, figsize=(16, 11.25))
        ...
        ## show x-tickes in 0-2
        axs_amps.set_xticks(np.arange(0, len(plot_data["stacked_amps"]) * 2 + 1, len(plot_data["stacked_amps"]) / 2))
        axs_amps.set_xticklabels(["0", "0.5", "1", "1.5", "2"])
